...
const counter = async () => {
  Sequence.findOne({}, (err, res) => {
    if (err) console.log('Error checking auto-increment sequence \n');
    else if (res === null) {
      Sequence({
        counter: 1000
      }).save((error, response) => {
        if (error) console.log('Error adding the auto-increment counter.');
        else console.log('Auto-increment counter set value :', response, '\n');
      });
    } else console.log('Auto-increment is already implemented. \n Current counter value :', res.counter);
  });
};
