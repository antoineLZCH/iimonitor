...
module.exports = {
    ...
    body: {
      user: Joi.array().items(
        Joi.object().keys({
          ...
          role: Joi.string().required().valid(['admin', 'user']).lowercase().trim()
        })
      )
    }
  },
  ...
  blockUnblock: {
    ...headers,
    body: {
      status: Joi.string().required().lowercase().trim().valid(['active', 'blocked']),
    },
    params: {
  ...
  login: {
    body: {
      clientType: Joi.string().valid('browser', 'ios', 'android').lowercase().trim().required(),
      deviceToken: Joi.string().optional().trim().default(''),
      email: Joi.string().required().lowercase().trim().label('Email or phone number'),
