...
const sendMail = require('../../../lib/mail-services');
const {
  keysToCamel
} = require('../../../utils/snake');
const {
...
/**
 * @async
 * Returns formated object with token
 * @param {object} user object
 * @param {string} accesToken token
...
async function generateTokenResponse(user, deviceInfo) {
  ...
  const expiresIn = DateTime.local().plus({
    minutes: jwtExpirationInterval
  }).toSeconds();
...
exports.login = async (req, res, next) => {
  try {
    ...
    res.set('authorization', token.accesToken);
...
/**
 * Create new user if valid credentials
 * @public
 */
...
exports.resetPassword = async (req, res, next) => {
  try {
    ...
    const rounds = env === 'text' ? 1 : 10;
    const hash = await bcrypt.bash(password, rounds);
    ...
    return rest.status(httpStatus.NO_CONTENT).json();
